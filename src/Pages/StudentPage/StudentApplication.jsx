...
const DOC_TYPES = [
  ...
  { key: "transcript", label: "Academic Transcript", required: false, icon: FileText },
  { key: "sop", label: "Statement of Purpose", required: false, icon: FileText },
  { key: "ielts", label: "IELTS / English Test", required: false, icon: BarChart },
  { key: "photo", label: "Passport Photo", required: false, icon: User },
  { key: "recommendation", label: "Recommendation Letter", required: false, icon: FileText },
  { key: "financial", label: "Financial Statement", required: false, icon: FileText },
  { key: "cv", label: "CV / Resume", required: false, icon: FileText },
  { key: "other", label: "Other Document", required: false, icon: FileText },
];

// Document Upload Modal
function DocumentUploadModal({ isOpen, onClose, onSuccess, application, docType }) {
 ...
 formData.append('doc_type', docType.key);
    ...
    try {
      const res = await authAxios.post(`${BASE_URL}/student/documents/upload`, formData);
      if (res.data) {
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-md w-full">
        <div className="p-5 border-b border-gray-100">
          ...
          <p className="text-sm text-gray-500">{docType?.label} for {application?.target_university}</p>
        </div>
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button type="submit" disabled={loading} className="flex-1 px-4 py-2 bg-teal-600 text-white rounded-xl hover:bg-teal-700">
              {loading ? <RefreshCw size={16} className="animate-spin mx-auto" /> : "Upload"}
            </button>
          </div>
        </form>
      </div>
...
function DocumentCard({ doc, onRefresh }) {
  ...
  const getStatusBadge = () => {
    switch (doc.status) {
      ...
      case 'review':
        return { bg: 'bg-blue-100', text: 'text-blue-700', icon: Clock, label: 'In Review' };
      default:
  ...
  console.log('Document:', { id: doc.id, fileUrl, fileName, status: doc.status });
    ...
    <div className="bg-gray-50 rounded-xl p-3 hover:shadow-sm transition">
      ...
      {isExpanded && (
        <div className="mt-3 pt-3 border-t border-gray-200 text-xs text-gray-500 space-y-1">
          <p><span className="font-medium">Submitted:</span> {new Date(doc.submitted_at).toLocaleDateString()}</p>
          {doc.reviewed_at && <p><span className="font-medium">Reviewed:</span> {new Date(doc.reviewed_at).toLocaleDateString()}</p>}
          {doc.rejection_reason && <p className="text-red-600"><span className="font-medium">Rejection Reason:</span> {doc.rejection_reason}</p>}
          {doc.notes && <p><span className="font-medium">Notes:</span> {doc.notes}</p>}
          {fileUrl && (
            <p className="text-blue-600">
              <a href={fileUrl} target="_blank" className="hover:underline">📄 Click to open document</a>
            </p>
          )}
        </div>
      )}
        ...
        <div className="flex justify-between items-start">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="font-bold text-gray-800 text-lg">{application.target_university || 'University'}</h3>
            </div>
            <div className="flex items-center gap-3 text-sm text-gray-500">
              <span className="flex items-center gap-1">
                <GraduationCap size={14} /> {application.course || 'Course not specified'}
              </span>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {DOC_TYPES.map((docType) => {
                ...
                let statusBadge = { bg: 'bg-gray-100', text: 'text-gray-500', label: 'Not Uploaded' };
                
                if (docStatus === 'verified') statusBadge = { bg: 'bg-green-100', text: 'text-green-700', label: 'Verified' };
                else if (docStatus === 'rejected') statusBadge = { bg: 'bg-red-100', text: 'text-red-700', label: 'Rejected' };
                else if (docStatus === 'review') statusBadge = { bg: 'bg-blue-100', text: 'text-blue-700', label: 'In Review' };
                else if (isUploaded) statusBadge = { bg: 'bg-amber-100', text: 'text-amber-700', label: 'Pending' };

                return (
                  <div key={docType.key} className="flex items-center justify-between p-2 bg-white rounded-lg border border-gray-100">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {!isUploaded && (
                        <button
                          onClick={() => setUploadModal({ isOpen: true, docType: docType })}
          ...
          <div>
            ...
            {loadingDocs ? (
              ...
              <div className="space-y-2">
                {documents.map((doc) => (
                  <DocumentCard key={doc.id} doc={doc} onRefresh={fetchDocuments} />
                ))}
              </div>
            )}
          </div>
    ...
    return (
      <div className="p-6 bg-slate-50 min-h-screen flex items-center justify-center">
        <div className="text-center">
          <GraduationCap className="text-teal-600 animate-pulse mx-auto mb-4" size={48} />
    ...
    <div className="p-4 bg-gradient-to-br from-slate-50 to-zinc-100 min-h-screen">
      ...
      <div className="mb-6">
        <div className="bg-gradient-to-r from-blue-950 to-teal-900 text-white rounded-2xl p-6 shadow-xl">
          <div className="flex justify-between items-center">
            <div>
              <p className="text-teal-300 text-xs font-semibold uppercase tracking-widest mb-1">Student Portal</p>
              <h1 className="text-2xl font-bold">My Applications</h1>
              <p className="text-blue-200 text-sm mt-1">View and track your university applications</p>
            </div>
            <button
      ...
      {applications.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-gray-100">
          ...
          <p className="text-xs text-gray-400 mt-1">Your counsellor will add applications for you</p>
        </div>
      ) : (
        <div className="space-y-4">
          {applications.map((app) => (
            <ApplicationCard key={app.id} application={app} onRefresh={fetchApplications} />
          ))}
        </div>
      )}
    </div>
