...
export default function Leads() {
  ...
  const fetchLeads = useCallback(async (page = 1) => {
    ...
    try {
      ...
      setLeads(leadsData);
      setPagination({ page, totalPages: data.totalPages || 1, total: data.total || leadsData.length });
    } catch (err) {
  ...
  async function handleSave(form) {
    ...
    const payload = { ...form, counsellor_id: form.counsellor_id ? Number(form.counsellor_id) : null };
    try {
      const res = await fetch(
        editLead ? `${BASE_URL}/admin/leads/${editLead.id}` : `${BASE_URL}/admin/leads`,
        {
  ...
  async function handleAssign(leadId, counsellor_id) {
    ...
    try {
      ...
      alert("Failed to assign counsellor");
      fetchLeads();
    }
  }
      ...
      <div className="flex-shrink-0 bg-white/80 backdrop-blur-sm border-b border-gray-100 px-6 py-4">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">{leads.length} total leads in pipeline</p>
          </div>
