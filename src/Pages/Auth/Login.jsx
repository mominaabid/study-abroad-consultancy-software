...
export default function Login() {
    ...
    useEffect(() => {
        ...
        if (params.get("session") === "expired") {
            ...
            // Show message for 5 seconds then auto-clear
    ...
    useEffect(() => {
        if (isAuth && role && ROLE_PATHS[role]) {
            // Double-check token isn't expired
    ...
    function handleSubmit(e) {
        ...
        // ✅ Auto-detect role based on email or use a selector
        // For now, let's use a simple check
        // You can add a role selector dropdown if needed
        const isCounsellor = email.includes('counsellor') || email.includes('cs@');
        
        if (isCounsellor) {
            dispatch(counsellorLogin({ email, password }));
        } else {
                        ...
                        <AnimatePresence mode="wait">
                            ...
                            {sessionExpired && (
                                <motion.div
                                    ...
                                    animate={{ opacity: 1, y: 0 }}
                                    exit={{ opacity: 0, y: -10 }}
                                    className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-xl text-yellow-700 text-sm flex items-center gap-2"
                                >
