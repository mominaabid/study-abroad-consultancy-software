...
import {
  addMessage, setMessages, markConversationRead,removeMessage, replaceMessage,
...
export default function ChatWindow({ conversation }) {
  ...
  // ── 1. Load history whenever conversation changes ──────────────────────────
  useEffect(() => {
    if (!convId) return;
   ...
   // ChatWindow.jsx - in the load history useEffect
.then(data => {
  ...
  // Then tell backend (fire and forget)
  ...
  }, [convId]); // ← only convId, not dispatch/user — those are stable
...
// ChatWindow.jsx - in the Ably subscription useEffect
subscribeToChannel(`conversation:${convId}`, 'new_message', (payload) => {
 ...
 async function sendMessage() {
  ...
  try {
    ...
    const realMsg = await res.json(); // backend returns the saved message
    
    // Replace temp with real message from backend response
