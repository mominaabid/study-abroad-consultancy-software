...
export const EditApplicationModal = ({
  ...
  const validateCnic = (value) => {
    ...
    if (hasLeadingSpaces(stringValue)) return "Cannot start with spaces";
    if (containsLetters(stringValue)) return "Cannot contain alphabets";
    const cleanCnic = stringValue.replace(/-/g, "");
  ...
  const validatePassport = (value) => {
    ...
    const stringValue = String(value);
    if (hasLeadingSpaces(stringValue)) return "Cannot start with spaces";
    if (!/^[A-Za-z0-9]{6,9}$/.test(stringValue)) {
  ...
  const validateCgpa = (value) => {
    ...
    if (!/^\d+(\.\d+)?$/.test(stringValue))
      return "Enter a valid CGPA (e.g., 3.5)";
    return null;
  };
  ...
  const validateTextOnlyField = (value, min = 3, max = 50) => {
    ...
    const stringValue = String(value);
    if (hasLeadingSpaces(stringValue)) return "Cannot start with spaces";
    if (containsNumbers(stringValue)) return "Cannot contain numbers";
    if (stringValue.length < min) return `Minimum ${min} characters required`;
    if (stringValue.length > max) return `Maximum ${max} characters allowed`;
    return null;
  };
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      ...
      case "cnic":
        return validateCnic(stringValue);
      ...
      case "counselor_notes": {
        ...
        if (hasLeadingSpaces(stringValue)) return "Cannot start with spaces";
        if (stringValue.length < 3) return "Minimum 3 characters required";
        if (stringValue.length > 250) return "Maximum 250 characters allowed";
        return null;
      }
